...
from transformers import PreTrainedTokenizer, PreTrainedModel, PreTrainedTokenizerFast
...
class DinoGenerator:
    ...
    def _generate_dataset_entries(
        self, input_text_or_id: Union[str, int], label: str, num_entries: int, generate_with_inputs: bool
    ) -> List[DatasetEntry]:
        ...
        if self.openai_api_key is not None:
            ...
            counter_instructions = [
                self._build_instruction(other_label, input_text_or_id, generate_with_inputs)
        ...
        model_outputs = [
            self._process_output(
                input_text=input_text_or_id, output_text=output, label=label, generate_with_inputs=generate_with_inputs
            )
    ...
    def _process_output(
        self, input_text: Union[str, int], output_text: str, label: str, generate_with_inputs: bool
        ...
        if self.min_num_tokens > 0:
            if generate_with_inputs:
                dataset = [
                    ...
                    if len(self.model._tokenizer.tokenize(entry.text_b)) >= self.min_num_tokens
                ]
            else:
                dataset = [
                    entry
                    for entry in dataset
                    if len(self.model._tokenizer.tokenize(entry.text_a)) >= self.min_num_tokens
                ]
...
class GPT2Wrapper(ModelWrapper):
    def __init__(self, model_name: str = "taeminlee/kogpt2", use_cuda: bool = True):
        """
        :param model_name: the name of the pretrained GPT2 model (default: "taeminlee/kogpt2)
        :param use_cuda: whether to use CUDA
        """
        super().__init__(use_cuda=use_cuda)
        # NOTE Customized for kogpt2
        self._tokenizer = PreTrainedTokenizerFast.from_pretrained(
            model_name, bos_token="</s>", eos_token="</s>", unk_token="<unk>", pad_token="<pad>", mask_token="<mask>"
        )
